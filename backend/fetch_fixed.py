...
try:
    ...
    for date_str in full_dates:
        ...
        # 转换日期格式 20260410 -> 2026.04.10
        dt = datetime.strptime(date_str, '%Y%m%d')
        display_date = f"{dt.year}.{dt.month:02d}.{dt.day:02d}"
        
        # 查询该日期的涨跌幅数据
        try:
            print(f"   正在查询: 同花顺板块 {display_date}")
            
            df = pywencai.get(
                query=f'同花顺板块 {display_date}',
                query_type='zhishu',
                cookie=cookie,
                loop=True,
                log=False
            ...
            if df is not None and len(df) > 0:
                ...
                print(f"   非0涨跌幅: {count_non_zero} / {count_inserted}")
